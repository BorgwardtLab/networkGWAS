...
class chi2mixture_sum(object):
    ...
    def fit_params(self):
        ...
        if self.isortlrt is None:
            ...
            self.qnulllrtsort = (1e-10 + sp.arange(self.mixture*self.isortlrt.shape[0]))/(self.mixture*self.isortlrt.shape[0])   
        ...
        #linear_constraint = LinearConstraint([[1, 1, 1, 1]],  [lowerlimit], [upperlimit])
    ...
    def fun(self, x):
        ...
        p = a*st.chi2.sf(self.lrtsort[:imax], 1) + b*st.chi2.sf(self.lrtsort[:imax], 2)# + \
            #+ c*st.chi2.sf(self.lrtsort[:imax], 3) + d*st.chi2.sf(self.lrtsort[:imax], 4)      
